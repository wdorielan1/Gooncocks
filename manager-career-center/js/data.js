/*
 * Data layer for the Manager Career Center.
 *
 * Everything on the page is calculated from the one league history this
 * returns (the same format the Rivalry Center uses) plus the payout rules
 * in data/payouts.js.
...
(function () {
  ...
  function normalize(raw) {
    ...
    if (skipped) console.warn('Career Center: skipped ' + skipped + ' matchup rows that did not match the expected format.');
    ...
    return {
      ...
      standings: standings,
      matchups: matchups,
      payouts: window.GOONCOCKS_PAYOUTS || null
    };
  }
  ...
  window.CareerData = { load: load, normalize: normalize };
})();
